...
/proc/set_security_level(var/level)
	...
	if(level >= SEC_LEVEL_GREEN && level <= SEC_LEVEL_DELTA && level != GLOB.security_level)
		...
		switch(level)
			if(SEC_LEVEL_GREEN)
				GLOB.security_announcement_down.Announce("Все угрозы станции были ликвидированы. Служба безопасности обязана снова убрать свое оружие, также законы приватности снова в силе, случайные обыски запрещены.","Внимание! Код безопасности был понижен до Зеленого.")
			...
			if(SEC_LEVEL_BLUE)
				if(GLOB.security_level < SEC_LEVEL_BLUE)
					GLOB.security_announcement_up.Announce("Станция получила достоверные доказательства о возможной враждебной активности на станции. Служба безопасности имеет право держать оружие на виду, а случайные обыски теперь дозволены.","Внимание! Код безопасности был повышен до Синего.")
				else
					GLOB.security_announcement_down.Announce("Непосредственная угроза была ликвидирована. Служба безопасности более не может держать оружие на полной боеготовности, однако все еще может держать его на виду. Случайные обыски все еще дозволены.","Внимание! Код безопасности был снижен до Синего.")
			...
			if(SEC_LEVEL_RED)
				if(GLOB.security_level < SEC_LEVEL_RED)
					GLOB.security_announcement_up.Announce("На станции присутствует непосредственная, подтвержденная, враждебная угроза. Служба безопасности имеет полное право держать оружие в полной боеготовности. Случайные обыски разрешены и рекомендуются.","Внимание! Код Красный!")
				else
					GLOB.security_announcement_down.Announce("Станционный механзм самоуничтожения был деактивирован, однако все еще присутствует непосредственная, подтвержденная, враждебная угроза активам и экипажу. Служба безопасности имеет право держать оружие в полной боеготовности. Случайные обыски разрешены и рекомендуются.","Внимание! Код Красный!")
			...
			if(SEC_LEVEL_EPSILON)
				GLOB.security_announcement_up.Announce("Центральное Командование авторизовало активацию кода Эпсилон на станции. Специальный элитный отряд быстрого реагирования будет отправлен для нейтрализации угрозы и защиты активов корпорации. Экипажу следует сохранять спокойствие и проследовать в ближайшее безопасное место на станции, а затем ожидать указаний от отряда быстрого реагирования.","Внимание! Код Эпсилон был активирован!", new_sound = sound('sound/effects/purge_siren.ogg'))
